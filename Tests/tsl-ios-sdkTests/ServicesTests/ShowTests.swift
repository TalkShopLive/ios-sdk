...
final class ShowTests: XCTestCase {
    ...
    func testClosedCaptions() {
        ...
        // When
        eventInstance.getDetails(showId: showId) { result in
            // Then
            switch result {
            case .success(let show):
                
                eventInstance.getClosedCaptions(showId: "vzzg6tNu0qOv") { result in
                    print("FILENAME with ShowId",result)
                }
                
                eventInstance.getClosedCaptions(eventId: 2472) { result in
                    print("FILENAME with EventId",result)
                }
